...
export default function ImageUpload(prop: any) {
    ...
    function onSubmit(data: any) {
        ...
        const form = new FormData()
        form.append('product_image', data.product_image[0]);
        fetch('http://localhost:4000/api/v1/product/image', {
            method: "POST",
            body: form,
            // headers:{
            //     "content-type":"multipart/form-data"
            ...
            if (res.ok) {
                ...
                prop.setImage(response.base64Image)
                        ...
                        <div className="text-center">
                            ...
                            <div className="mt-4 flex text-sm leading-6 text-gray-600">
                                <label htmlFor="product_image" className="relative cursor-pointer rounded-md bg-white font-semibold text-indigo-600 focus-within:outline-none focus-within:ring-2 focus-within:ring-indigo-600 focus-within:ring-offset-2 hover:text-indigo-500" >
                                    ...
                                    <input {...register("product_image")} onChange={handleFileChange} id="product_image" name="product_image" type="file" className="sr-only" />
                                </label>
                                <p className="pl-1">or drag and drop</p>
                            </div>
                            <p className="text-xs leading-5 text-gray-600">PNG, JPG, GIF up to 10MB</p>
                            <button onClick={() => prop.setImage(image)}>upload</button>
                        </div>
                    </div>
                </div>
                <div className="mt-3">
                   {image ? <ProductImages imageUrl={image.base64Image} removeImage={removeImage} /> : null}
                </div>
            </form>
        </div>
    );
