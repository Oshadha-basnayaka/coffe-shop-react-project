...
interface ProductImageUrlProp {
    imageUrl: string;
}

export function ProductImages({ imageUrl }: ProductImageUrlProp) {
    return (
        <div className="border flex justify-center p-2">
            <img className="w-[250px]" src={imageUrl} alt="Product" />
        </div>
    );
}
        ...
        <div>
            ...
            <div className="mt-3">
                {uploadedImages.map((imageUrl, index) => (
                    <ProductImages key={index} imageUrl={imageUrl} />
                ))}
            </div>
        </div>
    );
